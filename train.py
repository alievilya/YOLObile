...
def train(hyp):
    ...
    if opt.masked_retrain:
        ...
        if not opt.resume:
            # possible weights are '*.pt', 'yolov3-spp.pt', 'yolov3-tiny.pt' etc.
            print("\n>_ Loading file: ./model_pruned/yolov4_{}_{}_{}.pt".format(initial_rho * 10 ** (opt.rho_num - 1),
                                                                                opt.config_file, opt.sparsity_type))
            chkpt = torch.load(
                "./model_pruned/yolov4_{}_{}_{}.pt".format(initial_rho * 10 ** (opt.rho_num - 1), opt.config_file,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='head-train.data', help='*.data path')
    ...
    parser.add_argument('--admm-file', type=str, default='darknet_admm', help='admm configuration file')
    # parser.add_argument('--admm-file', type=str, default='admm', help='admm configuration file')

    opt = parser.parse_args()
    if (opt.admm_file):
        with open(os.path.join('cfg', opt.admm_file + ".yaml"), "r") as stream:
            try:
                raw_dict = yaml.load(stream, Loader=yaml.FullLoader)
                opt.admm = raw_dict['admm']
                opt.masked_retrain = raw_dict['masked-retrain']
                opt.admm_epochs = raw_dict['admm-epochs']
                opt.rho = raw_dict['rho']
                opt.rho_num = raw_dict['rho-num']
                opt.sparsity_type = raw_dict['sparsity-type']
